...
export default function AdvancedOpenWater() {
  const { data: course, isLoading, error } = useCourseDetail('advanced-open-water');
  ...
  return (
    <Layout>
      ...
      {/* Course Structure */}
      {course.requiredDives && course.optionalDives && (
        <section className="py-24">
          <div className="container max-w-5xl">
            <div className="text-center mb-12">
              <span className="text-xs font-bold uppercase tracking-widest text-primary">{course.courseStructureSection?.eyebrow}</span>
              <h2 className="text-3xl font-serif font-bold text-foreground mt-2">{course.courseStructureSection?.heading}</h2>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* Required Dives */}
              <div className="glass-panel p-8 space-y-6 border-primary/20">
                <h3 className="text-lg font-serif font-bold text-foreground">Required Dives</h3>
                <ul className="space-y-3">
                  {course.requiredDives.map((dive) => (
                    <li key={dive} className="flex items-center gap-3 text-sm text-foreground/90">
                      <CheckCircle2 className="w-4 h-4 text-primary shrink-0" />
                      {dive}
                    </li>
                  ))}
                </ul>
                <p className="text-xs text-muted-foreground">{course.courseStructureNote}</p>
              </div>

              {/* Popular Options */}
              <div className="glass-panel p-8 space-y-6">
                <h3 className="text-lg font-serif font-bold text-foreground">Popular Elective Options</h3>
                <ul className="space-y-3">
                  {course.optionalDives.map((dive) => (
                    <li key={dive} className="flex items-center gap-3 text-sm text-foreground/90">
