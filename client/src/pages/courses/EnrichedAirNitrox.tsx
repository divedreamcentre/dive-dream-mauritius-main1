...
export default function EnrichedAirNitrox() {
  const { data: course, isLoading, error } = useCourseDetail('nitrox');
    ...
    <Layout>
        ...
        <section className="py-24 bg-secondary/40">
          <div className="container max-w-5xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {course.learningPoints.map((benefit) => (
      ...
      {course.prerequisitesList && (
        <section className="py-24">
          <div className="container max-w-5xl">
            <div className="text-center mb-12">
              <span className="text-xs font-bold uppercase tracking-widest text-primary">{course.prerequisitesSection?.eyebrow}</span>
              <h2 className="text-3xl font-serif font-bold text-foreground mt-2">{course.prerequisitesSection?.heading}</h2>
            </div>
            <div className="max-w-3xl mx-auto space-y-4">
              {course.prerequisitesList.map((req) => (
                <div key={req} className="glass-panel p-6 flex items-start gap-4">
                  <AlertTriangle className="w-5 h-5 text-primary shrink-0 mt-0.5" />
                  <p className="text-sm text-foreground/90">{req}</p>
                </div>
              ))}
            </div>
          </div>
